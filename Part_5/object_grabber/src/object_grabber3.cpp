...
arm_motion_commander_(nodehandle), cart_move_action_client_("cartMoveActionServer", true) {
    ...
    switch (action_code) {
        ...
        case object_grabber::object_grabber3Goal::GRAB_OBJECT:
            ...
            if (grasp_option== object_grabber::object_grabber3Goal::DEFAULT_GRASP_STRATEGY) {
                ROS_INFO("using default strategy for object ID %d",object_id);
                approach_strategy=0;
                ...
                approach_strategy=goal->approach_strategy;
                lift_object_strategy = goal->lift_object_strategy;
                dropoff_strategy = goal->dropoff_strategy;
                dropoff_withdraw_strategy = goal->dropoff_withdraw_strategy;               
            }
            //get grasp-plan details for this case:
            grab_result_.return_code = object_grabber::object_grabber3Result::ACTION_CODE_UNKNOWN;
            ROS_INFO("returning action-code-unknown result");
            object_grabber_as_.setSucceeded(grab_result_);
            for (int i=0;i<5;i++) {
                ROS_INFO("pausing..");
