...
Eigen::Matrix4d Rrbot_fwd_solver::fwd_kin_solve_(Eigen::VectorXd q_vec) {
    ...
    //%compute A matrix frame i wrt frame i-1 for each joint:
...
Restore_IK_solver::Restore_IK_solver() {
    //constructor: 
    //ROS_INFO("Restore_IK_solver constructor");
}

//given angle of shoulder yaw (turret), compute the origin of frame2

Eigen::Vector3d Restore_IK_solver::get_frame2_origin_of_shoulder_yaw(double q_yaw) {
    Eigen::Matrix4d A = Eigen::Matrix4d::Identity();
...
bool Restore_IK_solver::solve_for_elbow_ang(Eigen::Vector3d w_wrt_0, double q_yaw, std::vector<double> &q_elbow_solns) {
...
bool Restore_IK_solver::solve_K_eq_Acos_plus_Bsin(double K, double A, double B, std::vector<double> &q_solns) {
...
bool Restore_IK_solver::fit_q_to_range(double q_min, double q_max, double &q) {
...
bool Restore_IK_solver::solve_for_humerus_ang(Eigen::Vector3d w_wrt_0, double q_yaw, double q_elbow, std::vector<double> &q_humerus_solns) {
...
bool Restore_IK_solver::solve_for_shoulder_pitch_ang(Eigen::Vector3d w_wrt_0, double q_yaw, double q_humerus, double q_elbow, std::vector<double> &q_shoulder_solns) {
    // z_wrist_wrt_1 = -d5*s3*s4 - c3*d4 +d2
    Eigen::Matrix4d A_1_wrt_0, A_w_wrt_1;
    Eigen::Matrix3d R;
    Eigen::Vector3d p, w_wrt_1;
...
bool Restore_IK_solver::ik_wrist_solns_of_q0(Eigen::Vector3d wrist_pt, double q_yaw, std::vector<Eigen::VectorXd> &q_solns) {
...
Eigen::Vector3d Restore_IK_solver::wrist_pnt_from_flange_frame(Eigen::Affine3d affine_flange_frame) {
...
bool Restore_IK_solver::solve_spherical_wrist(Vectorq7x1 q_in,Eigen::Matrix3d R_des, std::vector<Vectorq7x1> &q_solns) {
...
int Restore_IK_solver::ik_solve_given_qs0(Eigen::Affine3d const& desired_flange_pose_wrt_base,double q_yaw, std::vector<Vectorq7x1> &q_solns) {
...
int Restore_IK_solver::ik_solns_sampled_qs0(Eigen::Affine3d const& desired_flange_pose_wrt_base,std::vector<Vectorq7x1> &q_solns) {
       q_solns.clear();
        int n7dof_solns;
