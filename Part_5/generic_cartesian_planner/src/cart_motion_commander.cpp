...
// wsn, Nov, 2016
#include <generic_cartesian_planner/cart_motion_commander.h>
...
void CartMotionCommander::doneCb_(const actionlib::SimpleClientGoalState& state,
        const arm_motion_interface::cart_moveResultConstPtr& result) {
...
void CartMotionCommander::send_test_goal(void) {
    ...
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::ARM_TEST_MODE;
...
void CartMotionCommander::set_arrival_time_planned_trajectory(double arrival_time) {
  ...
  computed_arrival_time_ = arrival_time;
   cart_goal_.command_code = arm_motion_interface::cart_moveGoal::SET_ARRIVAL_TIME_PLANNED_TRAJECTORY;
        //case arm_motion_interface::cart_moveGoal::TIME_RESCALE_PLANNED_TRAJECTORY:
...
void CartMotionCommander::time_rescale_planned_trajectory(double time_scale_factor) {
   ...
   cart_goal_.command_code = arm_motion_interface::cart_moveGoal::TIME_RESCALE_PLANNED_TRAJECTORY;
        //case arm_motion_interface::cart_moveGoal::TIME_RESCALE_PLANNED_TRAJECTORY:
        //    time_scale_stretch_factor_ = goal->time_scale_stretch_factor;
...
int CartMotionCommander::plan_move_to_waiting_pose(void) {
    ...
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::PLAN_PATH_CURRENT_TO_WAITING_POSE;
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&CartMotionCommander::doneCb_, this, _1, _2)); // we could also name additional callback functions here, if desired
    //finished_before_timeout_ = cart_move_action_client_.waitForResult(ros::Duration(2.0));
    ROS_INFO("return code: %d",cart_result_.return_code);
    if (!cb_received_in_time(2.0)) {
            ROS_WARN("giving up waiting on result");
            return (int) arm_motion_interface::cart_moveResult::NOT_FINISHED_BEFORE_TIMEOUT;
        } 
    
    ROS_INFO("finished before timeout");
    if (cart_result_.return_code==arm_motion_interface::cart_moveResult::PATH_NOT_VALID) {
        ROS_WARN(" arm plan not valid");
        return (int) cart_result_.return_code;
    }
    if (cart_result_.return_code!=arm_motion_interface::cart_moveResult::SUCCESS) {
...
int CartMotionCommander::plan_jspace_path_current_to_qgoal(Eigen::VectorXd q_des_vec) {    
    ROS_INFO("requesting a joint-space motion plan");
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::PLAN_JSPACE_PATH_CURRENT_TO_QGOAL;
...
int CartMotionCommander::plan_path_current_to_goal_gripper_pose(geometry_msgs::PoseStamped des_pose) {
    ...
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::PLAN_PATH_CURRENT_TO_GOAL_GRIPPER_POSE;
    //ROS_INFO("des_pose: ");
...
int CartMotionCommander::plan_jspace_path_current_to_cart_gripper_pose(geometry_msgs::PoseStamped des_pose_gripper) {
    ROS_WARN("requesting a joint-space motion plan to cartesian gripper pose");
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::PLAN_JSPACE_PATH_CURRENT_TO_CART_GRIPPER_POSE; 
...
int CartMotionCommander::plan_path_current_to_goal_dp_xyz(Eigen::Vector3d dp_displacement) {
    ...
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::PLAN_PATH_CURRENT_TO_GOAL_DP_XYZ;
...
int CartMotionCommander::execute_planned_path(void) {
    ROS_INFO("requesting execution of planned path");
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::EXECUTE_PLANNED_PATH;
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&CartMotionCommander::doneCb_, this, _1, _2)); // we could also name additional callback functions here, if desired
...
int CartMotionCommander::request_q_data(void) {
   ROS_INFO("requesting arm joint angles");
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::GET_Q_DATA;
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&CartMotionCommander::doneCb_, this, _1, _2)); // we could also name additional callback functions here, if desired
    //finished_before_timeout_ = cart_move_action_client_.waitForResult(ros::Duration(computed_arrival_time_+2.0));
   if (!cb_received_in_time(computed_arrival_time_+2.0)) {
...
int CartMotionCommander::request_tool_pose(void) {
    // debug: compare this to output of:
    //rosrun tf tf_echo torso yale_gripper_frame
    ROS_INFO("requesting tool pose");    
    cart_goal_.command_code = arm_motion_interface::cart_moveGoal::GET_TOOL_POSE;
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&CartMotionCommander::doneCb_, this, _1, _2)); // we could also name additional callback functions here, if desired
    //finished_before_timeout_ = cart_move_action_client_.waitForResult(ros::Duration(2.0));
   if (!cb_received_in_time(2.0)) {
        ROS_WARN("did not respond within timeout");
