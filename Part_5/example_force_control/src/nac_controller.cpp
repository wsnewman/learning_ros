//attempt an NAC controller using emulated F/T sensor for feedback
...
const double  K_virt = 100.0; //1000 N/m virtual stiffness
const double  B_virt = 10.0; // 1N/(m/sec) virtual damping
const double v_ideal_sat = 1.0; // put a cap on max vel cmd
const double  x_attractor = -0.5;
...
int main(int argc, char **argv) {
   ...
   double f_virt = 0.0;
    ...
    while (ros::ok()) 
    {
        ...
        acc_ideal = f_net/M_virt; //test w/ f_virt; change to f_net
